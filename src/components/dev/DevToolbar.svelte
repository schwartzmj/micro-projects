<script lang="ts">
	let toolbarVisible = true;
</script>

<!-- Note: Custom class ".breakpoint" styles in project CSS file -->
<aside
	class="fixed bottom-1 right-1 z-50 flex divide-x divide-solid divide-white text-xs shadow-sm hover:opacity-100 motion-safe:transition-opacity"
	class:opacity-100={toolbarVisible}
	class:opacity-0={!toolbarVisible}
>
	<span
		title="Current Tailwind breakpoint"
		class="flex items-center space-x-1 bg-yellow-200 p-2 text-yellow-800"
	>
		<!-- Breakpoint SVG -->
		<svg class="h-4 w-4 fill-current" aria-hidden="true" viewBox="0 0 496.8 496.8">
			<path
				d="M118 377.8l-64-64c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l64 64c3.2 3.2 6.4 4.8 11.2 4.8s8-1.6 11.2-4.8c6.4-6.4 6.4-16 0-22.4z"
			/>
			<path
				d="M486 107.4l-96-96a39.7 39.7 0 00-54.4 0L10.8 334.6a39.6 39.6 0 000 54.4l96 96c8 8 17.6 11.2 27.2 11.2 9.6 0 19.2-3.2 25.6-11.2L486 161.8c14.4-16 14.4-40 0-54.4zm-24 30.4L137.2 462.6c-3.2 3.2-4.8 3.2-8 0l-96-96c-1.6-1.6-1.6-6.4 0-8L356.4 33.8c1.6-1.6 3.2-1.6 4.8-1.6 1.6 0 3.2 0 4.8 1.6l96 96c1.6 1.6 1.6 6.4 0 8z"
			/>
			<path
				d="M258.8 237l-64-64c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l64 64c3.2 3.2 6.4 4.8 11.2 4.8s8-1.6 11.2-4.8c6.4-6.4 6.4-16 0-22.4zm-72 22.4l-40-38.4c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l40 38.4c3.2 3.2 6.4 4.8 11.2 4.8s9.6-1.6 11.2-4.8c6.4-6.4 6.4-16 0-22.4zm-46.4 46.4L102 267.4c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l38.4 38.4c3.2 3.2 6.4 4.8 11.2 4.8 4.8 0 8-1.6 11.2-4.8 6.4-6.4 6.4-16 0-22.4zm187.2-187.2l-38.4-38.4c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l38.4 38.4c3.2 3.2 6.4 4.8 11.2 4.8 4.8 0 9.6-1.6 11.2-4.8 6.4-6.4 6.4-16 0-22.4zM281.2 165l-38.4-38.4c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l38.4 38.4c3.2 3.2 6.4 4.8 11.2 4.8s8-1.6 11.2-4.8c6.4-6.4 6.4-16 0-22.4zm118.4-68.8l-64-64c-6.4-6.4-16-6.4-22.4 0-6.4 6.4-6.4 16 0 22.4l64 64c3.2 3.2 6.4 4.8 11.2 4.8 4.8 0 8-1.6 11.2-4.8 6.4-6.4 6.4-16 0-22.4z"
			/>
		</svg>
		<span class="breakpoint" />
	</span>

	<button
		on:click={() => (toolbarVisible = !toolbarVisible)}
		title="Toggle toolbar visibility"
		aria-label="Toggle toolbar visibility"
		class="flex items-center justify-center bg-gray-200 px-2 hover:bg-gray-300"
	>
		<div class={`${toolbarVisible ? 'hidden' : 'block'}`}>
			<!-- Invisible SVG -->
			<svg class="h-5 w-5 fill-current" aria-hidden="true" viewBox="0 0 152 88">
				<path
					d="M116.4 53.3l6.6 12.2a3.5 3.5 0 106.3-3.4l-6.6-12.2a3.6 3.6 0 10-6.3 3.4zM97 59.5l.8-.1c1.6 0 3 1 3.5 2.7l3.4 13.5a3.6 3.6 0 11-7 1.7l-3.3-13.5c-.5-1.9.7-3.8 2.6-4.3zm-51.8 7.8a3.6 3.6 0 117 1.7l-3.5 13.5a3.5 3.5 0 01-4.3 2.6c-2-.5-3-2.4-2.6-4.3l3.4-13.5zm27.5-.9c2 0 3.5 1.6 3.5 3.6v13.8a3.6 3.6 0 01-7 0V70c0-2 1.5-3.6 3.5-3.6zm77.6-25.1a3.6 3.6 0 01-2.5 6.1c-.9 0-1.7-.3-2.4-1-.2-.1-5.2-4.8-12.9-10.7-16.1 11.8-41.1 26.4-66 26.4C42 62.1 24.1 48.4 13 36.7c-4 5-6.4 8.8-6.5 9a3.6 3.6 0 01-4.8 1.2 3.5 3.5 0 01-1.3-4.8c.3-.6 3.1-5.1 7.8-10.9A94 94 0 01.5 20.4c-1-1.7-.4-3.9 1.3-4.8 1.7-1 3.8-.4 4.8 1.3.1 0 2.5 3.9 6.5 9C24.1 14 42.1.3 66.4.3c25 0 50 14.6 66.1 26.4A186 186 0 00145.4 16a3.6 3.6 0 015 0 3.6 3.6 0 01-.1 5.1c-.6.5-5 4.7-11.9 10 6.8 5.4 11.4 9.6 11.9 10zm-24-10c-15.4-11-38-23.8-59.9-23.8-21.8 0-38.4 13-48.5 23.8C28.1 42 44.6 55 66.4 55c22 0 44.5-12.8 60-23.7zM19 51.9a3.5 3.5 0 013-1.9 3.6 3.6 0 013.2 5.3l-6.6 12.2a3.6 3.6 0 01-6.2-3.4L19 52z"
				/>
			</svg>
		</div>
		<div class={`${toolbarVisible ? 'block' : 'hidden'}`}>
			<!-- Visible SVG -->
			<svg class="h-5 w-5 fill-current" aria-hidden="true" viewBox="0 0 152 87">
				<path
					d="M116.4 34.8l6.6-12.2a3.5 3.5 0 116.3 3.3l-6.6 12.3a3.6 3.6 0 11-6.3-3.4zM97 28.5l.8.2c1.6 0 3-1.1 3.5-2.7l3.4-13.5a3.6 3.6 0 10-7-1.8l-3.3 13.5c-.5 2 .7 3.9 2.6 4.3zm-51.8-7.7a3.6 3.6 0 107-1.8L48.6 5.5A3.5 3.5 0 0044.4 3c-2 .5-3 2.5-2.6 4.3l3.4 13.6zm27.5.8c2 0 3.5-1.6 3.5-3.5V4a3.6 3.6 0 00-7 0v14c0 2 1.5 3.5 3.5 3.5zM150.3 66a3.6 3.6 0 01-2.5 6.1c-.9 0-1.7-.3-2.4-1-.2-.1-5.2-4.8-12.9-10.6-16.1 11.7-41.1 26.4-66 26.4C42 86.8 24.1 73 13 61.3c-4 5-6.4 8.8-6.5 9a3.6 3.6 0 01-4.8 1.2 3.5 3.5 0 01-1.3-4.8c.3-.5 3.1-5 7.8-10.8A94 94 0 01.5 45c-1-1.7-.4-3.8 1.3-4.8 1.7-1 3.8-.4 4.8 1.3.1 0 2.5 4 6.5 9 11-11.8 29-25.5 53.3-25.5 25 0 50 14.6 66.1 26.4a186 186 0 0012.9-10.7 3.6 3.6 0 015 .1 3.6 3.6 0 01-.1 5c-.6.5-5 4.7-11.9 10 6.8 5.4 11.4 9.6 11.9 10.1zm-24-10C111 44.9 88.4 32 66.5 32S28 45 18 55.9c10.2 10.8 26.7 23.8 48.5 23.8 22 0 44.5-12.9 60-23.8zM19 36a3.5 3.5 0 003 1.9 3.6 3.6 0 003.2-5.3l-6.6-12.2a3.6 3.6 0 00-6.2 3.4L19 36.1zm31 20a18 18 0 1036 0 18 18 0 00-36 0z"
				/>
			</svg>
		</div>
	</button>
</aside>

<style type="text/postcss">
	.breakpoint:before {
		@apply block uppercase content-['-'] sm:content-['sm'] md:content-['md'] lg:content-['lg'] xl:content-['xl'] 2xl:content-['2xl'];
	}
</style>
